...
_SYS_SYNTH = (
    "You are a research writer. You are given a STRUCTURED analysis as JSON "
    "(sources, verified claims, findings with observations). Your job is ONLY to "
    "organise and explain this evidence in prose. Return ONLY a JSON object with: "
    "executive_summary (string), problem_definition (string), recommendation "
    "(string), comparative_analysis (string), and reasoning (a list of objects "
    "with finding_id, interpretation, implication). "
    "STRICT RULES: base every sentence on the provided findings/claims; NEVER add "
    "facts, numbers, statistics, sources, or claims that are not in the input; do "
    "not contradict the verification status; if evidence is thin, say so."
)
...
async def build_report_evidence_first(
    ...
    # prose fields: grounded LLM output when available, else deterministic defaults
    ...
    report.recommendation = (synth.get("recommendation") or "").strip()
    report.comparative_analysis = (synth.get("comparative_analysis") or "").strip()
