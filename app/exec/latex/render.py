...
\fancyhead[R]{\footnotesize\sffamily\color{mute}Agentic AI OS}
\fancyfoot[L]{\footnotesize\sffamily\color{mute}Mission \#\MISSIONID}
\fancyfoot[C]{\footnotesize\sffamily\color{mute}AI-generated analysis, evidence-backed where applicable}
...
def _defs(report: Report) -> str:
    mid = str(report.meta.get("mission_id", "-"))
    return (f"\\newcommand{{\\REPTITLE}}{{{tex_escape(report.title)[:70]}}}\n"
            f"\\newcommand{{\\MISSIONID}}{{{tex_escape(mid)}}}\n")
...
def _cover(r: Report) -> str:
    ...
    lines = [
        ...
        r"{\sffamily\large\color{steel}\bfseries AGENTIC AI OS}\par\vspace{2pt}",
        r"{\sffamily\normalsize\color{mute}ANALYSIS REPORT}\par\vspace{20mm}",
    ...
    meta_rows = [
        ("Report type", rtype),
        ("Report date", str(meta.get("date", ""))),
        ("Mission ID", str(meta.get("mission_id", "-"))),
        ("Research status", str(status)),
        ("Evidence", f"{n_src} source(s)"),
    ]
    ...
    lines.append(body)
    lines.append(r"\vspace{6mm}\noindent{\footnotesize\color{mute}\sffamily Generated by "
                 r"Agentic AI OS $\cdot$ AI-generated analysis $\cdot$ evidence-backed where "
                 r"applicable.\par}")
...
def _exec_summary(r: Report) -> str:
    ...
    if r.coverage:
        ...
        out.append(r"\vspace{8pt}{\sffamily\footnotesize\color{mute}EVIDENCE COVERAGE}\par\vspace{3pt}"
                   ...
                   + _overall_conf(r).upper() + r"} (analyst assessment).}")
...
def _trail(r: Report) -> str:
    if not r.trail:
        return ""
    tr = r.trail
    out = [r"\section{Research Trail}",
           f"{tr.sources_used} source(s) used $\\cdot$ {tr.sources_excluded} excluded "
           f"$\\cdot$ last verified {tex_escape(tr.last_verified)}."]
    if tr.areas:
        items = "".join(r"\item " + tex_escape(a) for a in tr.areas)
        out.append(r"\begin{itemize}[leftmargin=14pt,itemsep=1pt,topsep=3pt]" + items + r"\end{itemize}")
    return "\n\n".join(out)
